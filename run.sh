...
docker build -t git.devmem.ru/projects/llm-homelab/llama.cpp:server-cuda -f .devops/cuda.Dockerfile --target server --build-arg CUDA_VERSION=$CUDA_VERSION --build-arg CUDA_DOCKER_ARCH=$CUDA_DOCKER_ARCH --progress=plain .
git reset --hard
cd ..
...
docker build -t git.devmem.ru/projects/llm-homelab/ik_llama.cpp:server-cuda -f .devops/llama-server-cuda.Dockerfile --build-arg CUDA_VERSION=$CUDA_VERSION --build-arg CUDA_DOCKER_ARCH=$CUDA_DOCKER_ARCH --progress=plain .
git reset --hard
cd ..
